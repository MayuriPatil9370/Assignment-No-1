...
namespace SGM_Student_Management_System
{
    public partial class frm_Add_New_Student : Form
    {
        ...
        int Auto_Incr()
        {
            ...
            Cmd.Connection = Con;
            Cmd.CommandText = "Select Count(*)From Student_Info";

            Cnt = Convert.ToInt32(Cmd.ExecuteScalar());

            if (Cnt > 0)
            {
                Cmd.CommandText = "Select Max(Roll_No)From Student_Info";
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            if (tb_Roll_No.Text != "" && tb_Name.Text != "" && tb_Mobile_No.Text != "" && cmd_Course.Text != "")
            {
                SqlCommand Cmd = new SqlCommand();
                Cmd.Connection = Con;
                Cmd.CommandText = "Insert Into Student_Info(Roll_No,Name,DOB,Mobile_No,Course)Values(@RNo,@Name,@DOB,@MobNo,@Course)";
